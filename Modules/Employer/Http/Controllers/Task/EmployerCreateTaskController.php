...
//        dd($validated);
        if (array_key_exists('CategoryAction', $validated)) {
            ...
            if (count($truest_requested_actions) != count($requested_actions)) {
                alert()->toast(trans('employer::task.An error occurred while selecting the operations to be performed on the required category, please try again'), 'error');
            ...
            alert()->toast(trans('employer::task.No work has been selected to be implemented on the requested platform, please select at least one'), 'error');
            return redirect()->back();
        }
        ...
        if (array_key_exists('daily_limit', $validated)) {
            ...
            if ($day_needed > $days_of_task) {
                alert()->toast(trans('employer::task.The time specified for the implementation of the project is not sufficient when setting this daily limit for the workers who will work on the task, so please increase the time for implementing the project or reduce the daily limit for workers'), 'warning');
                return redirect()->back();
            } else {
        ...
        if (array_key_exists('proof_request_ques', $validated)) {
            ...
            if ($check_request_length < 5) {
                alert()->toast(trans('employer::task.the question request test  must be not null or less than 5 CH'), 'error');
                return redirect()->back();
            }
        ...
        if (array_key_exists('proof_request_screenShot', $validated)) {
            ...
            if ($check_screenShot_length < 5) {
                alert()->toast(trans('employer::task.the screenShot request test must be not null or less than 5 CH'), 'error');
        ...
        if (count($cities_ids) > count(collect($cities_ids)->unique())) {
            alert()->toast(trans('employer::task.this task cities must be unique'), 'error');
            return redirect()->back();
...
//           This means that the type of discount was on the payment fees and therefore there is no change to the final value of the job
            ...
            if ($employer->wallet_balance >= $total_cost) {
                ...
                alert()->toast(trans('employer::task.The task has been successfully created and deducted from your wallet balance'), 'success');
                ...
                alert()->toast(trans('employer::task.The task has been created successfully, but your wallet does not have enough funds to pay for the task, you can review the task and pay for it from the task section task basket'), 'warning');
...
//           This means that the type of discount was on the payment fees and therefore there is no change to the final value of the job
            ...
            alert()->toast(trans('employer::task.this value is minimum cost per worker for region and category selected'), 'error');
