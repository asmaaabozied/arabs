...
//                    update worker wallet balance
                    ...
                    alert()->toast(trans('worker::worker.The money has been transferred to your PayPal account successfully'), 'success');
...
//                       This means that the (ArabWorkers) wallet balance does not have enough money to pay the required amount
                        alert()->toast(trans('worker::worker.The payment process was not possible for some reason. Please try again later. If the status continues for more than 24 hours, inform the support department immediately.'), 'error');
                        return redirect()->route('worker.show.my.wallet.history');
                    } else {
                        alert()->toast(trans('worker::worker.The payment process was not possible for some reason. Please try again later. If the status continues for more than 24 hours, inform the support department immediately.'), 'error');
                        return redirect()->route('worker.show.my.wallet.history');
...
//                    dd($e,4);
                ...
                alert()->toast(trans('worker::worker.Your balance is not sufficient to withdraw this amount'), 'error');
                return redirect()->route('worker.show.my.withdraw.using.paypal.form');
            }
        }
