        ...
        if ($task->daily_limit == null){
            ...
            if ($task->only_professional == "false") {
                ...
                if ($check_if_ordered == 0) {
                    if (array_sum($check_if_worker_levies_in_any_task_city) == 1)
                        if ($task->total_worker_limit > $task->approved_workers) {
                            ...
                            alert()->toast(trans('worker::task.You have successfully ordered to completed this task'), 'success');
                            return redirect()->route('worker.tasks.in.active');
                        } else {
                            alert()->toast(trans('worker::task.Enough workers have come forward to accomplish this task'), 'error');
                            return redirect()->back();
                        }
                    else {
                        alert()->toast(trans('worker::task.you are not living in any of task city'), 'warning');
                        return redirect()->back();
                    }

                } else {
                    alert()->toast(trans('worker::task.have already applied for this task'), 'error');
                    return redirect()->back();
                }
            } else {
                /**
                ...
                if ($worker_level == "Silver" or $worker_level == "Golden") {
                     ...
                     */
                    alert()->toast(trans('worker::task.This task is for professional workers only'), 'error');
                    return redirect()->back();
                }
            }
...
//            dd($count_of_today_worker, $worker_limit,$worker_limit > $count_of_today_worker);
            if ($worker_limit > $count_of_today_worker){
                ...
                alert()->toast(trans('worker::task.The number of workers who applied to complete this task has reached the required limit, try to apply to complete the task again at 00:01 am local time'), 'warning');
                return redirect()->back();
            }
    ...
    public function showTaskInActiveDetails($task_id, $task_number)
    {
        ...
        if ($task_statuses->last()->status->name == "active") {
            ...
            alert()->toast(trans('worker::task.An error has occurred Please try again later'), 'error');
            return redirect()->back();
        }
    ...
    public function finishTaskJob(FinishTaskJobRequest $request, $task_id, $task_number)
    {
        ...
        if (isset($check_uploaded_proof) and count($check_uploaded_proof) >= 1) {
            alert()->toast(trans('worker::task.I have actually completed the task and sent the proof of completion earlier'), 'warning');
            return redirect()->back();
        } else {
            ...
            alert()->toast(trans('worker::task.Proof of completion has been sent successfully'), 'success');
