...
class VerifyAccountController extends Controller
{
    ...
    public function SendEmailVerification()
    {
        ...
        if (count($checkLimitToken) <= 4) {
            ...
            alert()->toast(trans('worker::worker.The confirmation email has been sent, check your inbox or spam and click on the confirmation email option'), 'success');
            return redirect()->route('worker.show.my.profile');
        } else {
            alert()->toast(trans('worker::worker.You have exceeded the account activation limit of times, and therefore your account has been suspended Contact our support team or create a new account'), 'error');
            return redirect()->route('worker.show.my.profile');
        }
    }
    ...
    public function verifyAccount($token, $worker_number)
    {
        ...
        if ($worker == null){
            alert()->toast(trans('worker::worker.An error occurred while confirming the email, please try again later'), 'error');
            return redirect()->route('worker.show.my.profile');
        }
        if ($worker->email_verified_at != null){
            alert()->toast(trans('worker::worker.You have already confirmed your email and you do not need to do this procedure again'), 'info');
            return redirect()->route('worker.show.my.profile');
        }
        if ($worker->temporaryToken->last()->token == $token) {
            ...
            alert()->toast(trans('worker::worker.Email has been confirmed successfully'), 'success');
            return redirect()->route('worker.show.my.profile');
        } else {
            $worker->temporaryToken()->forceDelete();
            alert()->toast(trans('worker::worker.An error occurred while confirming the email, please try again later'), 'error');
...
//        alert()->toast(trans('worker::worker.A text message has been sent to the phone number registered with us. Check the message and enter the code sent into the required box.'), 'success');
        alert()->toast(trans('worker::worker.This service is not currently active'), 'info');
        return redirect()->route('worker.show.my.profile');
    }
}
